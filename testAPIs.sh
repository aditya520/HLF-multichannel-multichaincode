...
function setChaincodePath(){
	...
	case "$LANGUAGE" in
		"golang")
		CC_SRC_PATH="chaincode_example02/go"
		;;
		"node")
...
curl -s -X POST \
  ...
  -H "content-type: application/json" \
  -d '{
	"configUpdatePath":"../channel-artifacts/Org1MSPanchors.tx"
}'
echo
echo
...
curl -s -X POST \
  ...
  -H "content-type: application/json" \
  -d '{
	"configUpdatePath":"../channel-artifacts/Org2MSPanchors.tx"
}'
echo
echo
